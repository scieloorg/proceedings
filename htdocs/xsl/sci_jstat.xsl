...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="STATISTICS_BY_JOURNAL">
		<html>
			<head>
				<title>
					<xsl:value-of select="TITLEGROUP/TITLE" disable-output-escaping="yes"/> - <xsl:call-template name="PrintPageTitle"/>
				</title>
							...
							<p align="center">
								<a>
									<xsl:call-template name="AddScieloLink">
										...
										<xsl:with-param name="seq" select="TITLEID"/>
									</xsl:call-template>
									<img>
										<xsl:attribute name="src">http://<xsl:value-of select="CONTROLINFO/SCIELO_INFO/SERVER"/>/<xsl:value-of select="CONTROLINFO/SCIELO_INFO/PATH_SERIMG"/><xsl:value-of select="TITLEGROUP/SIGLUM"/>/plogo.gif</xsl:attribute>
										<xsl:attribute name="alt"><xsl:value-of select="TITLEGROUP/SHORTTITLE"/></xsl:attribute>
								...
								<blockquote>
									<p>
										<font class="nomodel" color="#0000A0">
											<b>
												<xsl:call-template name="PrintUsageReportTitle"/>
											</b>
										</font>
									</p>
								</blockquote>
										...
										<li>
											<a>
												<xsl:call-template name="AddScieloLogLink">
													...
													<xsl:with-param name="pid" select="TITLEID"/>
												</xsl:call-template>
												<xsl:choose>
	...
	<xsl:template name="CreateBibLink">
		...
		<xsl:param name="wh1"/>
		<xsl:param name="wh2"/>
		<xsl:attribute name="href">http://<xsl:value-of select="//CONTROLINFO/SCIELO_INFO/SERVER"/><xsl:value-of select="//CONTROLINFO/SCIELO_INFO/PATH_CGI-BIN"/><xsl:value-of select="$script"/>?usr=fbpe&amp;lng=<xsl:value-of select="//CONTROLINFO/LANGUAGE"/>&amp;pid=<xsl:value-of select="$pid"/>&amp;wh1=<xsl:value-of select="$wh1"/>&amp;wh2=<xsl:value-of select="$wh2"/></xsl:attribute>
	</xsl:template>
				...
				<td width="65%">
					<blockquote>
						<p>
							<font class="nomodel" color="#0000A0">
								<b>
									<xsl:call-template name="PrintCitationTitle"/>
								</b>
							</font>
						</p>
					</blockquote>
		...
		<li>
			...
			<br/> - <a>
						<xsl:call-template name="CreateBibLink">
							<xsl:with-param name="pid" select="/STATISTICS_BY_JOURNAL/TITLEID"/>
	...
	<xsl:template match="HALF_LIFE">
		<xsl:if test="count(YEAR) != 0">
			<li>
				...
				<br/> - <xsl:apply-templates select="YEAR">
