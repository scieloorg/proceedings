...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="HeadStatJournal">
	   ...
	   Most visited
				...
				<xsl:value-of select="$entities_en"/>
			</TITLE>
			<style type="text/css">
	...
	<xsl:template name="FormStatJournal">
		...
		<TABLE cellpadding="0" cellspacing="0" width="100%">
					...
					<P align="center">
						<A>
							...
							<IMG border="0">
								...
								<xsl:attribute name="alt"><xsl:choose><xsl:when test="//TITLEGROUP"><xsl:value-of select="//TITLEGROUP/SHORTTITLE "/></xsl:when><xsl:otherwise><xsl:value-of select="//SITE_NAME"/></xsl:otherwise></xsl:choose></xsl:attribute>
							</IMG>
						</A>
					</P>
				</TD>
				...
				<TD>
					<BLOCKQUOTE>
						<FONT face="Verdana" size="2" color="#800000">
							<xsl:choose>
								<xsl:when test="//CONTROLINFO/LANGUAGE='en'">Most visited <xsl:value-of select="$entities1_en"/>
								</xsl:when>
								<xsl:when test="//CONTROLINFO/LANGUAGE='es'">
									<xsl:value-of select="$entities1_es"/> más visitados</xsl:when>
								<xsl:when test="//CONTROLINFO/LANGUAGE='pt'">
									<xsl:value-of select="$entities1_pt"/> mais visitados</xsl:when>
							</xsl:choose>
						</FONT>
						<BR/>
						<P>
           ...
           (<xsl:value-of select="$measured_by_line1_en"/>
										<xsl:if test="$measured_by_line2_en">
											<BR/>
											<xsl:value-of select="$measured_by_line2_en"/>
										</xsl:if>)
        </xsl:when>
									<xsl:when test="//CONTROLINFO/LANGUAGE='es'">
           (<xsl:value-of select="$measured_by_line1_es"/>
										<xsl:if test="$measured_by_line2_es">
											<BR/>
											<xsl:value-of select="$measured_by_line2_es"/>
										</xsl:if>) 
        </xsl:when>
									<xsl:when test="//CONTROLINFO/LANGUAGE='pt'">
           (<xsl:value-of select="$measured_by_line1_pt"/>
										<xsl:if test="$measured_by_line2_pt">
											<BR/>
											<xsl:value-of select="$measured_by_line2_pt"/>
										</xsl:if>) 
        </xsl:when>
								</xsl:choose>
							</FONT>
   ...
   1</OPTION>
	...
	<xsl:template name="AddScieloLogLinkCall">
		...
		<xsl:call-template name="AddScieloLogLink">
			...
			<xsl:with-param name="pid" select="/STATISTICS/TITLEID"/>
		</xsl:call-template>
	</xsl:template>
</xsl:stylesheet>
