...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="PutOrderSelector">
		...
		<xsl:param name="TIP"/>
		<xsl:choose>
			<xsl:when test=" not(//FILTER/ORDER) "/>
			<xsl:when test=" //FILTER/ORDER = $ORDER ">
				<b>
			...
			<xsl:otherwise>
				<a>
					<xsl:call-template name="AddScieloLogLink">
						<xsl:with-param name="pid" select="//JOURNAL/ISBN"/>
					...
					<b>
						<font face="Symbol" color="#0000eb" size="2">Ñ</font>
					</b>
				</a>
			</xsl:otherwise>
		</xsl:choose>
	...
	<xsl:template match="JOURNAL">
		<tr>
			<xsl:attribute name="bgcolor"><xsl:choose><xsl:when test="(position() mod 2) = 1">#f2f2f2</xsl:when><xsl:otherwise>#eff7f7</xsl:otherwise></xsl:choose></xsl:attribute>
...
<xsl:when test="LIST or not(JOURNAL)">
									<xsl:attribute name="href">http://<xsl:value-of select="CONTROLINFO/SCIELO_INFO/SERVER"/><xsl:value-of select="CONTROLINFO/SCIELO_INFO/PATH_DATA"/>scielo.php?lng=<xsl:value-of select="CONTROLINFO/LANGUAGE"/>&amp;nrm=<xsl:value-of select="CONTROLINFO/STANDARD"/></xsl:attribute>
									<img>
										<xsl:attribute name="src">http://<xsl:value-of select="CONTROLINFO/SCIELO_INFO/SERVER"/><xsl:value-of select="CONTROLINFO/SCIELO_INFO/PATH_DATA"/><xsl:value-of select="CONTROLINFO/SCIELO_INFO/PATH_GENIMG"/><xsl:value-of select="CONTROLINFO/LANGUAGE"/>/fbpelogp.gif</xsl:attribute>
								...
								<xsl:otherwise>
									<xsl:call-template name="AddScieloLink">
										<xsl:with-param name="seq" select="JOURNAL/ISBN"/>
										<xsl:with-param name="script">sci_serial</xsl:with-param>
									</xsl:call-template>
									<img>
										<xsl:attribute name="src">http://<xsl:value-of select="CONTROLINFO/SCIELO_INFO/SERVER"/><xsl:value-of select="CONTROLINFO/SCIELO_INFO/PATH_DATA"/><xsl:value-of select="CONTROLINFO/SCIELO_INFO/PATH_SERIMG"/><xsl:value-of select="JOURNAL/TITLEGROUP/SIGLUM"/>/plogo.gif</xsl:attribute>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								...
								<font class="nomodel" color="#000080">
									<xsl:apply-templates select="JOURNAL/ISBN">
		...
		<tr bgColor="#dfebeb" width="100%">
			<xsl:if test="LIST">
				<td width="30%">
					...
					<xsl:if test="LIST">
						<br/>
						<xsl:call-template name="PutOrderSelector">
