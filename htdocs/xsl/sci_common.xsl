...
-->
	...
	<xsl:template name="AddIAHLink">
		...
		<xsl:param name="base">
			<xsl:value-of select="//CONTROLINFO/SCIELO_INFO/databases/db[@type='article' and @lang='en']"/>
		</xsl:param>
		<xsl:attribute name="href">http://<xsl:value-of select="//CONTROLINFO/SCIELO_INFO/SERVER"/><xsl:value-of select="//CONTROLINFO/SCIELO_INFO/PATH_WXIS"/><xsl:value-of select="//CONTROLINFO/SCIELO_INFO/PATH_DATA_IAH"/>?IsisScript=<xsl:value-of select="//CONTROLINFO/SCIELO_INFO/PATH_CGI_IAH"/>iah.xis&amp;base=<xsl:value-of select="$base"/><xsl:if test="$scope">^d<xsl:value-of select="$scope"/></xsl:if>&amp;<xsl:if test="$index">index=<xsl:value-of select="$index"/>&amp;</xsl:if>format=<xsl:value-of select="//CONTROLINFO/STANDARD"/>.pft&amp;lang=<xsl:choose><xsl:when test="//CONTROLINFO/LANGUAGE='en'">i</xsl:when><xsl:when test="//CONTROLINFO/LANGUAGE='es'">e</xsl:when><xsl:when test="//CONTROLINFO/LANGUAGE='pt'">p</xsl:when></xsl:choose><xsl:if test="$scope and $scope!='library'">&amp;limit=<xsl:value-of select="//TITLEID"/></xsl:if></xsl:attribute>
	</xsl:template>
	<!-- Shows Title Group -->
	<xsl:template match="ISSUE|STRIP" mode="titlegroup">
		<CENTER>
		...
		<CENTER>
			<FONT color="#000080">
				...
				<xsl:if test="count(ISBN)=0">
					<xsl:apply-templates select="PUBL/ISBN">
						<xsl:with-param name="LANG" select="//CONTROLINFO/LANGUAGE"/>
					</xsl:apply-templates>
				</xsl:if>
			</FONT>
	...
	<!-- Gets the type of the ISBN
         Parameters:
           TYPE - Type Code (PRINT | CDROM | DISKE | ONLIN)  
           LANG - language code   -->
	<xsl:template name="GET_ISBN_TYPE">
		<xsl:param name="TYPE"/>
		<xsl:param name="LANG"/>
		<xsl:choose>
	...
	<xsl:template match="FORMERTITLE">
		<xsl:choose>
			<xsl:when test="TITLE/@TITLEID">
				<a>
					<xsl:call-template name="AddScieloLink">
						<xsl:with-param name="seq" select="TITLE/@TITLEID"/>
							...
							<xsl:when test="$SERIAL">
								<font color="#000000">
									<xsl:call-template name="GET_ISBN_TYPE">
										<xsl:with-param name="TYPE" select="@TYPE"/>
										<xsl:with-param name="LANG" select="$LANG"/>
									</xsl:call-template>&#160;ISBN</font>&#160;<xsl:value-of select="normalize-space(.)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:call-template name="GET_ISBN_TYPE">
									<xsl:with-param name="TYPE" select="@TYPE"/>
									<xsl:with-param name="LANG" select="$LANG"/>
								</xsl:call-template>&#160;ISBN&#160;<xsl:value-of select="normalize-space(.)"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:when test=" $LANG='pt' ">
...
-->
	...
	<xsl:template name="PrintArticleInformationLabel">
		<xsl:param name="LANGUAGE"/>
		<xsl:choose>
			<xsl:when test=" $LANGUAGE = 'en' ">How to cite this paper</xsl:when>
			<xsl:when test=" $LANGUAGE = 'pt' ">Como citar este trabalho</xsl:when>
			<xsl:when test=" $LANGUAGE = 'es' ">Como citar este trabajo</xsl:when>
		</xsl:choose>
	</xsl:template>
