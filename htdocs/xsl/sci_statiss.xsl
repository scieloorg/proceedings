...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<HTML>
			...
			<BODY bgcolor="#FFFFFF">
				...
				<TABLE border="0" align="center" cellpadding="2" cellspacing="2">
					<xsl:attribute name="width"><xsl:choose><xsl:when test="count(//ISSUE_LIST/ISSUE)>0">70%</xsl:when><xsl:otherwise>80%</xsl:otherwise></xsl:choose></xsl:attribute>
	...
	<xsl:template match="ISSUE">
		<TR width="70%">
			<xsl:attribute name="bgcolor"><xsl:choose><xsl:when test="position() mod 2 != 0">#F2F2F2</xsl:when><xsl:otherwise>#EFF7F7</xsl:otherwise></xsl:choose></xsl:attribute>
