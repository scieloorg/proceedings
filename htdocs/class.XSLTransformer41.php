...
class XSLTransformer {
	...
	{
		call_user_func_array(array($this, 'XSLTransformer'), func_get_args());
	}

/* Constructor  */	 
	function XSLTransformer() {
	    $this->defFile = parse_ini_file(dirname(__FILE__)."/scielo.def.php", true, INI_SCANNER_RAW);
...
*/
    ...
    function transform()
    {
		...
		if ($this->validateXML($this->xml)){
			...
			if ($tryRedirect){
					// Disabled legacy self-redirect on transform error (causes loop on PHP 8).
			} else {
	...
	function setError ($string) { 
 	...
 	function getError() { 
		return isset($this->error) ? $this->error : 0; 
	} 
...
class docReader { 
	...
	function __construct()
	{
		call_user_func_array(array($this, 'docReader'), func_get_args());
	}
