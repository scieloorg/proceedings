...
class ScieloBase
{
	...
	{
		call_user_func_array(array($this, 'ScieloBase'), func_get_args());
	}
	...
	function _TransformXML()
	{
		...
		if($transform->setXsl($this->_xsl))
		{
   			if($transform->setXml($this->_xml))
			{
				...
				{
						$result = "<p>Error transforming: " . htmlspecialchars((string)$transform->getError(), ENT_QUOTES) . "</p>\n";
				}
			}
			else
			{
		...
		$currentHost = isset($_SERVER["HTTP_HOST"]) ? $_SERVER["HTTP_HOST"] : "scielo-proceedings";
		$baseUrl = "http://" . $currentHost;
		$result = str_replace("http://www.proceedings.scielo.br", $baseUrl, $result);
		$result = str_replace("https://www.proceedings.scielo.br", $baseUrl, $result);
		$result = str_replace("http://scielo-proceedings", $baseUrl, $result);
